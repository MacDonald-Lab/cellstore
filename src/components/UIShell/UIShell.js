...
const UIShell = ({ organizationName, libraries }) => {
  ...
  const MenuItems = (props) => (
    <>
      <HeaderMenu
        aria-label="Link 4"
        menuLinkName="Libraries"
        isCurrentPage={location.pathname === "/library"}
      >
        {libraries.map((item) => (
          <HeaderMenuItem
            key={item.name}
      ...
      <HeaderMenuItem
        isCurrentPage={location.pathname === "/computations"}
        element={Link}
        to="/computations"
        onClick={props.onClick}
      >
    ...
    <HeaderContainer
      render={({ isSideNavExpanded, onClickSideNavExpand }) => (
        <Header aria-label="CellSTORE Header">
          ...
          />
          <HeaderName element={Link} to="/" prefix={organizationName}>
            CellSTORE
          </HeaderName>
          ...
          {userAgent.indexOf(" electron/") > -1 && (
            <div className="bx--header__draggable ui-shell__electron-draggable"></div>
          )}

          <HeaderGlobalBar>
            <HeaderGlobalAction
              aria-label="Log out"
              className="ui-shell__logout-button"
              onClick={() => {
